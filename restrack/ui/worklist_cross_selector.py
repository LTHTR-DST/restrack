...
def get_all_worklists():
    ...
    try:
        ...
        print("Fetching worklists")  # Debug logging
...
def unsubscribe(worklists_to_remove):
    ...
    if worklists_to_remove:
        ...
        try:
            unsubscribe_data = {
                "user_id": pn.state.cache["current_user"]["id"],
                "worklist_ids": worklists_to_remove
                if isinstance(worklists_to_remove, list)
                else [worklists_to_remove],  # Ensure it's a list
            }
            unsubscribe_worklists = json.dumps(unsubscribe_data)
            print(f"Sending unsubscribe request with data: {unsubscribe_data}")

            r = requests.delete(
                f"{API_URL}/unsubscribe_worklist/{unsubscribe_worklists}"
            )
            r.raise_for_status()
...
def refresh_subscribed_worklists():
    ...
    user_id = pn.state.cache["current_user"][
        "id"
    ]  # Fix: use current_user["id"] instead of user_id
    try:
...
def update_subscribed_worklists(new_worklists_names):
    ...
    if new_worklists_names:
        ...
        new_worklist_ids = [
            lookup.get(name) for name in new_worklists_names
        ]  # Each item is a tuple of (id, name)
...
def worklist_cross_selector():
    ...
    worklist_cross_selector_widget = pn.widgets.CrossSelector(
        name="Subscribe and Unsubscribe Worklists",
        value=subscribed_worklists,
...
def worklist_manager():
    ...
    worklist_manager_component = pn.Column(
        cross_selector_component, btn_update_worklists
    )
